...
(function(){'use strict';
 ...
 function embeddedTeacherGrid(t){if(!t)return '<div class="r18628-state bad">No approved timetable is available for this account.</div>';const M=window.GSM_R18628_TIMETABLE_MASTER||{},days=['MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY'];return '<div class="r18628-source"><b>CURRENT TIMETABLE · '+esc(M.academic_year||'2026-2027')+' · '+esc(M.term||'TERM 1')+'</b><span>Approved timetable master fallback is active because Supabase returned no published rows.</span></div><div class="r18628-week-wrap"><table class="r18628-week"><thead><tr><th>PERIOD / HOUR</th>'+days.map(d=>'<th>'+d+'</th>').join('')+'</tr></thead><tbody>'+(t.slots||[]).map(s=>'<tr><th><b>'+esc(s.label)+'</b><span>'+esc(s.time||'')+'</span></th>'+days.map((d,i)=>'<td>'+esc((s.days||[])[i]||'—').replace(/\n/g,'<br>')+'</td>').join('')+'</tr>').join('')+'</tbody></table></div>'}
 window.GSM_R18697_embeddedTeacherGrid=embeddedTeacherGrid;
 ...
 V.myteachertimetable=function(mount,ctx){
   ...
   async function load(){body.innerHTML='<div class="r18628-state">Loading timetable…</div>';try{const d=await rpc('r136_teacher_timetable',{});rows=Array.isArray(d&&d.rows)?d.rows:[];if(rows.length){draw();return}const M=window.GSM_R18628_TIMETABLE_MASTER||{};const p=profile(),name=norm(p.full_name||p.username||'');const keys=Object.keys(M.teacher_timetables||{});let k=keys.find(x=>norm(x)===name);if(!k&&p.email)k=keys.find(x=>String(M.teacher_timetables[x]?.email||'').trim().toLowerCase()===String(p.email).trim().toLowerCase());if(!k&&p.phone)k=keys.find(x=>String(M.teacher_timetables[x]?.phone||'').replace(/\D/g,'')===String(p.phone).replace(/\D/g,''));if(k){body.innerHTML=embeddedTeacherGrid(M.teacher_timetables[k]);scheduleUi(body);return}draw()}catch(e){const M=window.GSM_R18628_TIMETABLE_MASTER||{};const p=profile(),name=norm(p.full_name||p.username||'');const k=Object.keys(M.teacher_timetables||{}).find(x=>norm(x)===name);if(k){body.innerHTML=embeddedTeacherGrid(M.teacher_timetables[k]);scheduleUi(body);return}body.innerHTML='<div class="r18628-state bad"><b>TIMETABLE LOAD FAILED.</b><span>'+esc(e&&e.message||e)+'</span></div>'}}
 ...
 function embeddedManagerBrowser(view,cs,ts){const A=window.GSM_R18632_AUTHORITY||{},M=window.GSM_R18628_TIMETABLE_MASTER||{},days=['MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY'];const classes=Object.keys(A.class_timetables||{}),teachers=Object.keys(M.teacher_timetables||{});cs.innerHTML=classes.map(x=>'<option value="'+esc(x)+'">'+esc(x)+'</option>').join('');ts.innerHTML=teachers.map(x=>'<option value="'+esc(x)+'">'+esc(x)+'</option>').join('');const showClass=()=>{const code=cs.value,rows=(A.class_timetables||{})[code]||[],slots=[],seen=new Map();rows.forEach(r=>{const k=String(r[1])+'|'+String(r[2]);if(!seen.has(k)){seen.set(k,{label:r[1],time:r[2],days:{}});slots.push(seen.get(k))}seen.get(k).days[r[0]]={activity:r[3],teacher:r[4]}});view.innerHTML='<div class="r18628-source"><b>APPROVED CURRENT TIMETABLE · '+esc(A.academic_year||M.academic_year||'2026-2027')+' · '+esc(A.term||M.term||'TERM 1')+'</b><span>Supabase has no published rows; approved timetable master is shown for continuity.</span></div><div class="r18628-week-wrap"><table class="r18628-week"><thead><tr><th>PERIOD / HOUR</th>'+days.map(d=>'<th>'+d+'</th>').join('')+'</tr></thead><tbody>'+slots.map(s=>'<tr><th><b>'+esc(s.label)+'</b><span>'+esc(s.time)+'</span></th>'+days.map(d=>{const v=s.days[d];return '<td>'+esc(v?v.activity:'—')+(v&&v.teacher?'<br><small>'+esc(v.teacher)+'</small>':'')+'</td>'}).join('')+'</tr>').join('')+'</tbody></table></div>';};const showTeacher=()=>{view.innerHTML=embeddedTeacherGrid(M.teacher_timetables&&M.teacher_timetables[ts.value]);scheduleUi(view)};if(!cs.value&&classes.length)cs.value=classes[0];if(!ts.value&&teachers.length)ts.value=teachers[0];$('#r81ShowClass',host).onclick=showClass;$('#r81ShowTeacher',host).onclick=showTeacher;showClass()}
 window.GSM_R18697_embeddedManagerBrowser=embeddedManagerBrowser;
 ...
 function injectLiveManager(mount){
   ...
   async function load(){try{data=await rpc('r120_get_timetable_workspace',{});const classes=(data.classes||[]).filter(x=>String(x.status||'ACTIVE').toUpperCase()!=='ARCHIVED'),staff=(data.staff||[]).filter(x=>String(x.status||'ACTIVE').toUpperCase()==='ACTIVE');cs.innerHTML=classes.map(x=>'<option value="'+esc(x.id)+'">'+esc(x.class_code||x.class_name||'CLASS')+'</option>').join('');ts.innerHTML=staff.map(x=>'<option value="'+esc(x.id)+'">'+esc(x.full_name||x.staff_code||'TEACHER')+'</option>').join('');const src=$('.r18681-live-source span',host);if(src)src.textContent='LIVE SUPABASE · '+(data.entries||[]).filter(e=>e.is_active!==false&&norm(e.status)==='PUBLISHED').length+' published rows';if((data.entries||[]).length)drawClass();else if(typeof window.GSM_R18697_embeddedManagerBrowser==='function')window.GSM_R18697_embeddedManagerBrowser(view,cs,ts);else drawClass()}catch(e){if(typeof window.GSM_R18697_embeddedManagerBrowser==='function')window.GSM_R18697_embeddedManagerBrowser(view,cs,ts);else view.innerHTML='<div class="r18628-state bad"><b>LIVE TIMETABLE BROWSER FAILED.</b><span>'+esc(e&&e.message||e)+'</span></div>'}}
...
(function(){
'use strict';
if(window.__GSM_R18697_PRINT_SAFE__)return;window.__GSM_R18697_PRINT_SAFE__=true;
function direct(){try{if(typeof window.GSM_printReport==='function')return window.GSM_printReport({title:'CURRENT VIEW',landscape:true});}catch(_){}try{const n=window.__GSM_R18670_NATIVE_PRINT;if(typeof n==='function')return n();}catch(_){}try{window.print()}catch(e){alert('PRINT FAILED: '+(e&&e.message||e))}}
document.addEventListener('click',function(ev){const b=ev.target.closest&&ev.target.closest('#r81Print,#r83PrintClass,#r83PrintTeacher,#r83PrintAllClasses,#r83PrintAllTeachers');if(!b)return;/* Existing timetable handler remains authoritative when it can create its own document. */if(b.id==='r81Print')return;setTimeout(function(){},0)},true);
window.GSM_R18697_SAFE_PRINT=direct;
})();
